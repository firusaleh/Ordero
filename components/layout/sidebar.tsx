...
export default function Sidebar() {
  ...
  return (
    <div className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-64 lg:flex-col rtl:right-0 rtl:left-auto">
      <div className="flex grow flex-col gap-y-5 overflow-y-auto border-r rtl:border-r-0 rtl:border-l border-gray-200 bg-white px-6 pb-4">
                    ...
                    <li key={item.name}>
                      <Link
                        ...
                        className={cn(
                          ...
                          'group flex gap-x-3 rtl:gap-x-reverse rounded-md p-2 text-sm leading-6 font-medium transition-colors'
                        )}
                      >
                        <Icon
                          className={cn(
                            pathname === item.href ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600',
                            'h-5 w-5 shrink-0 transition-colors rtl:ml-3 rtl:mr-0'
                          )}
                        />
                        {item.name}
                      </Link>
